...
static __device__ uint32_t mixtab0c[256] = {
...
static void SMIX_LDG(const uint32_t shared[4][256], uint32_t &x0,uint32_t &x1,uint32_t &x2,uint32_t &x3){
	uint32_t c0 = __ldg(&mixtab0c[__byte_perm(x0,0,0x4443)]);
	uint32_t r1 = mixtab1(__byte_perm(x0,0,0x4442));
	uint32_t r2 = mixtab2(__byte_perm(x0,0,0x4441));
	uint32_t r3 = mixtab3(__byte_perm(x0,0,0x4440));
	c0 = c0 ^ r1 ^ r2 ^ r3;
	uint32_t r0 = __ldg(&mixtab0c[__byte_perm(x1,0,0x4443)]);
	uint32_t c1 = r0 ^ mixtab1(__byte_perm(x1,0,0x4442));
	uint32_t tmp = mixtab2(__byte_perm(x1,0,0x4441));
	c1 ^= tmp;
	r2 ^= tmp;
	...
	c1 ^= tmp;
	r3 ^= tmp;
	uint32_t c2 = __ldg(&mixtab0c[__byte_perm(x2,0,0x4443)]);
	r0 ^= c2;
	tmp = mixtab1(__byte_perm(x2,0,0x4442));
	c2 ^= tmp;
	r1 ^= tmp;
	...
	c2 ^= tmp;
	r3 ^= tmp;
	uint32_t c3 = __ldg(&mixtab0c[__byte_perm(x3,0,0x4443)]);
	r0 ^= c3;
	tmp = mixtab1(__byte_perm(x3,0,0x4442));
	c3 ^= tmp;
	r1 ^= tmp;
	...
	c3 ^= tmp;
	r2 ^= tmp;
	tmp = ROL8(__ldg(&mixtab0c[__byte_perm(x3,0,0x4440)]));
	c3 ^= tmp;
	x0 = ((c0 ^ (r0 << 0)) & 0xFF000000) | ((c1 ^ (r1 << 0)) & 0x00FF0000) | ((c2 ^ (r2 << 0)) & 0x0000FF00) | ((c3 ^ (r3 << 0)) & 0x000000FF);
	x1 = ((c1 ^ (r0 << 8)) & 0xFF000000) | ((c2 ^ (r1 << 8)) & 0x00FF0000) | ((c3 ^ (r2 << 8)) & 0x0000FF00) | ((c0 ^ (r3 >>24)) & 0x000000FF);
	x2 = ((c2 ^ (r0 <<16)) & 0xFF000000) | ((c3 ^ (r1 <<16)) & 0x00FF0000) | ((c0 ^ (r2 >>16)) & 0x0000FF00) | ((c1 ^ (r3 >>16)) & 0x000000FF);
...
#define TPB_F 256
__global__ __launch_bounds__(TPB_F,3)
void xevan_fugue512_gpu_hash_128(uint32_t threads, uint64_t *g_hash)
{
	...
	if (thread < threads)
	{
        ...
        if(threadIdx.x<256){
            const uint32_t tmp = mixtab0c[threadIdx.x];
...
//		__syncthreads();
		...
		#pragma unroll 16
		for(int i =0; i<16;i++)Hash[i]=cuda_swab32(Hash[i]);
...
__syncthreads();
        ...
        FUGUE512_3(Hash[0xF], 0, 0);
        FUGUE512_3(0, 0, 0);
        FUGUE512_3(0, 0, 0);
        FUGUE512_3(0, 0, 0);
        FUGUE512_3(0, 0, 0);
        
        uint2 fc_bit_count = vectorize((uint64_t)128 << 3);
        FUGUE512_3(0, 0, fc_bit_count.y);
        FUGUE512_1(fc_bit_count.x);

        #define SWAP_3(a,b,c) { uint32_t u = a; a = b; b = c; c = u; }
        SWAP_3(S[0], S[24], S[12]);
        SWAP_3(S[1], S[25], S[13]);
        SWAP_3(S[2], S[26], S[14]);
        SWAP_3(S[3], S[27], S[15]);
        SWAP_3(S[4], S[28], S[16]);
        SWAP_3(S[5], S[29], S[17]);
        SWAP_3(S[6], S[30], S[18]);
        SWAP_3(S[7], S[31], S[19]);
        SWAP_3(S[8], S[32], S[20]);
        SWAP_3(S[9], S[33], S[21]);
        SWAP_3(S[10], S[34], S[22]);
        SWAP_3(S[11], S[35], S[23]);
...
__host__
void xevan_fugue512_cpu_hash_128(int thr_id, uint32_t threads, uint32_t *d_hash){

	const uint32_t threadsperblock = TPB_F;

	// berechne wie viele Thread Blocks wir brauchen
	dim3 grid((threads + threadsperblock-1)/threadsperblock);
	dim3 block(threadsperblock);

	xevan_fugue512_gpu_hash_128<<<grid, block>>>(threads, (uint64_t*)d_hash);
}