...
void xevan_sha512_gpu_hash_128(int threads,  uint64_t *g_hash)
{
...
__host__ void xevan_sha512_cpu_hash_128(int thr_id, uint32_t threads, uint32_t *d_hash)
{
	const int threadsperblock = TPB_SHA;

	dim3 grid((threads + threadsperblock-1)/threadsperblock);
	dim3 block(threadsperblock);
	xevan_sha512_gpu_hash_128<<<grid, block>>>(threads,  (uint64_t*)d_hash);
}
